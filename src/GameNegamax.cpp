...
int Game::findBestMove()
{
	...
	for (int i = 0; i < board.getCols(); i++)
	{
		...
		if (board.isColumnEmpty(i))
		{
			...
			// get score
			int score = this->negamax(1, maxDepth, false, player); //this->alphabetapruning(1,maxDepth,INT_MIN,INT_MAX,false,player);
			// update score
			if (score > bestScore)
			{
				bestScore = score;
	...
	if (isMaximizingPlayer)
	{
		...
		for (int i = 0; i < board.getCols(); i++)
		{
			...
			if (board.isColumnEmpty(i))
			{
				...
				board.makeMove(i, player);
				// get score
				int score = negamax(depth + 1, maxDepth, false, nextPlayer);
				// update score
				score=-score;
				bestScore = max(bestScore, score);
				// undo move
	...
	else
	{
		...
		for (int i = 0; i < board.getCols(); i++)
		{
			...
			if (board.isColumnEmpty(i))
			{
				...
				board.makeMove(i, player);
				// get score
				int score = this->negamax(depth + 1, maxDepth, true, nextPlayer);
				// update score
				score=-score;
				bestScore = min(bestScore, score);
				// undo move
